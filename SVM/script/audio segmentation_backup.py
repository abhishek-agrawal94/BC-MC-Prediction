...
def extract_time_v3(transcription):
    audio_name = transcription[:-4] + '.wav'  
    audio = AudioSegment.from_file(audio_name)
    audio_length = audio.duration_seconds * 1000
    original_txt = pd.read_csv(transcription,delimiter='(\:\s)|(\]\s)',header=None, on_bad_lines='skip')
...
def extract_time_info_v1(transcription):
    audio_name = transcription[:-4] + '.wav'  
    audio = AudioSegment.from_file(audio_name)
    audio_length = audio.duration_seconds * 1000
    original_txt = pd.read_csv(transcription,delimiter='(\:\s)|(\]\s)',header=None, on_bad_lines='skip')
...
# Version 2: For the starting point and text in different lines 
def extract_time_info_v2(transcription):
    audio_name = transcription[:-4] + '.wav'  
    audio = AudioSegment.from_file(audio_name)
    audio_length = audio.duration_seconds * 1000
    original_txt = pd.read_csv(transcription,delimiter='\:\s',header=None, on_bad_lines='skip')
...
def extract_time_info_v3(transcription):
    audio_name = transcription[:-4] + '.wav'  
    audio = AudioSegment.from_file(audio_name)
    audio_length = audio.duration_seconds * 1000
    original_txt = pd.read_csv(transcription,delimiter='(\:\s)|(\]\s)',header=None, on_bad_lines='skip')
...
def get_time(path,version):
    ...
    for transcription in file_lst:
        if version == 'v1':
            data = extract_time_info_v1(transcription)
        elif version == 'v2':
            data = extract_time_info_v2(transcription)
        else:
            data = extract_time_info_v3(transcription)
        final = pd.concat([data,final])
    return final

path = 'D:\\course_material\\thesis\\BC\\modeling\\dataset\\forced alignment\\FA materials_new\\trans_2' 
...
path = 'D:\\course_material\\thesis\\BC\\modeling\\dataset\\forced alignment\\FA materials_new\\textgrid'
transcription = utt
# transcription = pd.read_csv('utterance.csv')
layername = "ORT-MAU"
folder = os.fsencode(path)
problem = []
final = pd.DataFrame()
